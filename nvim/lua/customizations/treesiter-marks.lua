...
--
-- local function get_context(buf_nr, win_id)
-- 	return require("treesitter-context.context").get(buf_nr, win_id)
-- end
--
-- local context, context_lines = get_context(bufnr, winid)
--
-- local function open(buf_nr, win_id, ctx_ranges, ctx_lines)
-- 	require("treesitter-context.render").open(buf_nr, win_id, ctx_ranges, ctx_lines)
-- end
--
...
local required_marks = {
	["'"] = true,
	["a"] = true,
	["b"] = true,
}

local function removeLeadingWhitespace(str)
	local _, endIndex = string.find(str, "%S")
	if endIndex then
		return string.sub(str, endIndex)
	else
		return "" -- String is either empty or contains only whitespace
	end
end

local function get_marks()
	local filtered_mark_list = {}
	local mark_list = vim.fn.getmarklist(bufnr)
	for _, mark_item in pairs(mark_list) do
		local mark = mark_item.mark:sub(2, 3)
		print(mark)
		if required_marks[mark] then
			table.insert(filtered_mark_list, {
				[1] = mark_item.pos[2],
				[2] = 0,
				[3] = mark_item.pos[2],
				[4] = -1,
...
local function get_mark_lines(marks)
	...
	for _, mark_item in pairs(marks) do
		local mark_line = mark_item[1]
		local value = vim.api.nvim_buf_get_lines(bufnr, mark_line - 1, mark_line, true)[1]
		local cured_value = removeLeadingWhitespace(value)
		table.insert(mark_lines, cured_value)
	end
	return mark_lines
end
-- print_table(vim.fn.getmarklist(bufnr))
local marks = get_marks()
-- print("marks")
-- print_table(marks)
local mark_lines = get_mark_lines(marks)

---@class Position
---@field [1] integer end row
---@field [2] integer end row
---@field [3] integer end row
---@field [4] integer end row

---@class Mark
---@field mark string mark sign
---@field value string start column

-- print("context")
-- print_table(context)
-- print("context_lines")
-- print_table(context_lines)
--
-- local row = 2
-- local new_context = {
-- 	[1] = {
...
-- 	},
-- }
-- local new_context_lines = {
-- 	[1] = "local bufnr = api.nvim_get_current_buf()",
-- }
-- open(bufnr, winid, new_context, new_context_lines)
--
...
-- -- context_lines
--
-- -- 1 local function print_table(table)
-- -- 2 ^Ifor k, v in ipairs(table) do
...
local function render_marks(win, bufnr, contexts, gutter_width)
...
local function render_lines(bufnr, ctx_bufnr, ctx_ranges, ctx_lines, win_height)
...
function M.open(bufnr, winid, ctx_ranges, ctx_lines)
	...
	render_marks(winid, gbufnr, ctx_ranges, gutter_width)
	...
	render_lines(bufnr, ctx_bufnr, ctx_ranges, ctx_lines, win_height)
end
...
M.open(bufnr, winid, marks, mark_lines)
print(context_winid)
return M
